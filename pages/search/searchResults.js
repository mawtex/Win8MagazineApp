    ...
    ui.Pages.define(searchPageURI, {
        ...
        ready: function (element, options) {
            ...
            listView.addEventListener("loadingstatechanged", function (e) {
                ...
                for (var i = 0; i < images.length; i++) {
                    var img = images[i];
                    img.style.display = (img.complete ? 'inherit' : 'none'); // hide broken images - nice in offline mode
                    console.log(img.src);
                }
            });
        ...
        _initializeLayout: function (listView, viewState) {
            ...
            if (viewState === appViewState.snapped) {
                ...
                document.querySelector(".titlearea .pagetitle").textContent = c1metro.options.appTitle; //"App Name";
        ...
        _itemInvoked: function (args) {
            args.detail.itemPromise.done(function itemInvoked(item) {
                WinJS.Navigation.navigate("/pages/itemDetail/itemDetail.html", { item: Data.getItemReference(item.data) });
            });
        },
        ...
        _searchData: function (queryText) {
            ...
            if (window.Data) {
                originalResults = Data.items.createFiltered(function (item) {
                    return ((item.title && item.title.toLowerCase().indexOf(queryText) >= 0) || (item.subtitle && item.subtitle.toLowerCase().indexOf(queryText) >= 0) || (item.description && item.description.toLowerCase().indexOf(queryText) >= 0));
                });
            } else {
